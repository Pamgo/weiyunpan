...
.all-icon {
	background-image: url(<%=path%>/imgs/frame-icon.png);
}
...
.s-icon {
	background: url(<%=path%>/imgs/aside-app-icon.png);
...
.a_isfavor{cursor: pointer;color:green;}
.a_isfavor:hover{color:red;}
</style>
</head>
...
<body>
	<div class="content">
					...
					<div class="feed-panel">
						<div class="feed-content">
							...
							<c:forEach items="${shareLists }" var="isshareFile">
								<ul id="con-ul" class="column"
									style="width: 403px; height: auto;">
									<li class="con-ul-li">
										<div class="f-name">
											<span class="file-title">文件名称：</span> 
											<span class="file-desc">
											<a class="file_name" href="<%=path %>/FileDownloadServlet.do?method=downloadFile&filename=${isshareFile.filename }&fileid=${isshareFile.id}"
												title="点击下载文件">${isshareFile.filename }</a>&nbsp;&nbsp;(点击下载)</span>
										</div>
										...
										<div class="share-time">
											<span class="file-title">分享日期：</span> <span class="file-desc">${isshareFile.change_time
												}</span>
										</div>
										<div class="share-time">
											...
											<%-- <a class="a_isfavor" href = "<%=path %>/FileFavoriteServlet.do?method=andfavorForward&fileid=${isshareFile.id}" title="收藏文件"><span class="file-title">收藏</span></a>
											 --%>
											<a class="a_isfavor" title="收藏文件"><span
												class="file-title">收藏</span> </a> <span class="file-desc"></span>
										</div>
									</li>
								</ul>
							</c:forEach>
		...
		<!--
        	...
        	描述：main
        	-->
		<div class="main" id="layoutmain"></div>
	</div>
	...
	<script type="text/javascript">
		$(function() {
			$(".a_isfavor").click(function() {
				...
				window.confirm("确定要收藏"+filename+"文件吗？");
